...
export default async (req: Request, context: Context) => {
  ...
  const [weekly, overrides, appointments] = await Promise.all([
    ...
    d.sql`SELECT id,customer_name,email,phone,date::text AS date,start_time,duration_minutes,treatment_id,treatment_name,price_cents,status,source,created_at
